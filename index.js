...
/* fetch("https://pokeapi.co/api/v2/pokemon/ditto")
 ...
 async function getPokemon() {
    // Getting the search input element and its value
    const searchInput = document.getElementById("searchelement");
    const searchValue = searchInput.value.toLowerCase(); // Convert the input to lowercase
  
    try {
      // Fetching data from the Pokémon API using the user input
      const response = await fetch(https://pokeapi.co/api/v2/pokemon/${searchValue});
  
      // Checking if the resource is available
      if (!response.ok) {
        throw new Error("The Pokémon could not be found.");
      }
      ...
      const pokemonImage = data.sprites.front_default; // Getting the Pokémon image from the API response
      document.querySelector("img").src = pokemonImage; // Setting the image source in the HTML
  ...
  document.getElementById("searchme").addEventListener("click", getPokemon);